...
// 🔧 IMPORTANT: tell qr-scanner where the worker file is
...
function ScanPage() {
  ...
  useEffect(() => {
    ...
    if (scannerRef.current) return; // avoid double-init in React StrictMode
    ...
    const scanner = new QrScanner(
      ...
      (result) => {
        ...
        try {
          ...
          // if it's NOT a URL, treat the whole data as the docId
        ...
        router.push(`/track/materials/details?id=${docId}`);
      },
      {
        highlightScanRegion: true,
        highlightCodeOutline: true,
        preferredCamera: "environment", // prefer back camera on mobiles
      }
    );
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#F7E8FF] via-[#FDFBFF] to-[#E4D4FF] flex flex-col items-center px-4 py-6">
      ...
      <div className="w-full max-w-md flex items-center justify-between mb-4">
        ...
        <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-white/80 border border-purple-100 text-[10px] font-semibold text-[#A259FF]">
          <span className={`h-1.5 w-1.5 rounded-full ${scanning ? "bg-green-500" : "bg-gray-400"}`} />
      ...
      <p className="mt-3 text-[10px] text-gray-500 text-center max-w-xs">
        If the camera doesn’t open, ensure browser camera permission is allowed and
